...
class Chapters extends Table {
  TextColumn get id => text()();
  TextColumn get audioId => text().references(AudioItems, #id)();
...
class Transcripts extends Table {
  ...
  IntColumn get startMs => integer()();
  IntColumn get endMs => integer()();
  IntColumn get offsetAdjust => integer().withDefault(const Constant(0))();
...
class Vocabulary extends Table {
  ...
  IntColumn get sourceOffsetMs => integer().withDefault(const Constant(0))();
  DateTimeColumn get createdAt => dateTime().withDefault(currentDateAndTime)();
...
class WordMemory extends Table {
  TextColumn get wordId => text().references(Vocabulary, #id)();
  ...
  IntColumn get quizCorrect => integer().withDefault(const Constant(0))();
  RealColumn get masteryLevel => real().withDefault(const Constant(0.0))();
...
class ContentMemory extends Table {
  TextColumn get audioId => text().references(AudioItems, #id)();
  IntColumn get chaptersHeard => integer().withDefault(const Constant(0))();
  IntColumn get wordsQueried => integer().withDefault(const Constant(0))();
...
class AgentSessions extends Table {
  ...
  TextColumn get audioId => text().nullable().references(AudioItems, #id)();
  TextColumn get chapterId => text().nullable().references(Chapters, #id)();
...
class ReviewSchedule extends Table {
  ...
  IntColumn get reviewCount => integer().withDefault(const Constant(0))();
  TextColumn get lastResult => text().withDefault(const Constant(''))();
...
class WeaknessProfile extends Table {
  IntColumn get id => integer().autoIncrement()();
  TextColumn get wordId => text().references(Vocabulary, #id)();
  TextColumn get weakType => text().withDefault(const Constant('vocabulary'))();
...
class LearningPatterns extends Table {
  TextColumn get id => text()();
  TextColumn get estimatedLevel => text().withDefault(const Constant('B1'))();
  TextColumn get levelBasis => text().withDefault(const Constant(''))();
  TextColumn get activeHours => text().withDefault(const Constant(''))();
  IntColumn get avgSessionMin => integer().withDefault(const Constant(0))();
  TextColumn get preferredTopics => text().withDefault(const Constant(''))();
